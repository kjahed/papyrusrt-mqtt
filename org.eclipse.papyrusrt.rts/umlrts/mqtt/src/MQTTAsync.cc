		...
		int persistence_type, void* persistence_context,  MQTTAsync_createOptions* options)
{
	...
	m = (MQTTAsyncs*) malloc(sizeof(MQTTAsyncs));
	*handle = m;
	...
	if (options)
	{
		m->createOptions = (MQTTAsync_createOptions*) malloc(sizeof(MQTTAsync_createOptions));
...
static MQTTAsync_queuedCommand* MQTTAsync_restoreCommand(char* buffer, int buflen)
{
	...
	FUNC_ENTRY;
	qcommand = (MQTTAsync_queuedCommand*) malloc(sizeof(MQTTAsync_queuedCommand));
	...
	switch (command->type)
	{
		case SUBSCRIBE:
			...
			for (i = 0; i < command->details.sub.count; ++i)
			{
				...
				command->details.sub.topics[i] = (char*) malloc(data_size);
		...
		case UNSUBSCRIBE:
			...
			for (i = 0; i < command->details.unsub.count; ++i)
			{
				...
				command->details.unsub.topics[i] = (char*) malloc(data_size);
		...
		case PUBLISH:
			data_size = strlen(ptr) + 1;
			command->details.pub.destinationName = (char*) malloc(data_size);
...
int MQTTAsync_reconnect(MQTTAsync handle)
{
	...
	MQTTAsyncs* m = (MQTTAsyncs*) handle;
	...
	else
	{
		...
		MQTTAsync_queuedCommand* conn = (MQTTAsync_queuedCommand*) malloc(sizeof(MQTTAsync_queuedCommand));
...
static int MQTTAsync_processCommand(void)
{
	...
	*/
	...
	else if (command->command.type == PUBLISH)
	{
		...
		p->payload = (char*) command->command.details.pub.payload;
...
static void nextOrClose(MQTTAsyncs* m, int rc, char* message)
{
	if (MQTTAsync_checkConn(&m->connect, m))
	{
		...
		/* put the connect command back to the head of the command queue, using the next serverURI */
		conn = (MQTTAsync_queuedCommand*) malloc(sizeof(MQTTAsync_queuedCommand));
		memset(conn, '\0', sizeof(MQTTAsync_queuedCommand));
		conn->client = m;
		conn->command = m->connect;
		Log(TRACE_MIN, -1, "Connect failed, more to try");
			...
			if (pack)
			{
				if (pack->header.bits.type == CONNACK)
				{
					...
					if (rc == MQTTASYNC_SUCCESS)
					{
						...
						if (m->connected)
						{
							char* reason = (char*) ((onSuccess) ? "connect onSuccess called" : "automatic reconnect");
						...
						if (command->command.token == ((Suback*)pack)->msgId)
						{
							...
							else if (command->command.onSuccess)
							{
								...
								else if (sub->qoss->count > 1)
								{
									...
									int* element = array = data.alt.qosList = (int*) malloc(sub->qoss->count * sizeof(int));
...
void Protocol_processPublication(Publish* publish, Clients* client)
{
	...
	FUNC_ENTRY;
	mm = (MQTTAsync_message*) malloc(sizeof(MQTTAsync_message));
	...
	{
		qEntry* qe = (qEntry*) malloc(sizeof(qEntry));
		qe->msg = mm;
...
int MQTTAsync_connect(MQTTAsync handle, const MQTTAsync_connectOptions* options)
{
	...
	if (options->struct_version >= 2 && options->serverURIcount > 0)
	{
		...
		m->serverURIs = (char**) malloc(options->serverURIcount * sizeof(char*));
...
static int MQTTAsync_disconnect1(MQTTAsync handle, const MQTTAsync_disconnectOptions* options, int internal)
{
	...
	dis = (MQTTAsync_queuedCommand*) malloc(sizeof(MQTTAsync_queuedCommand));
...
int MQTTAsync_subscribeMany(MQTTAsync handle, int count, char* const* topic, int* qos, MQTTAsync_responseOptions* response)
{
	...
	sub = (MQTTAsync_queuedCommand*) malloc(sizeof(MQTTAsync_queuedCommand));
	...
	sub->command.details.sub.topics = (char**) malloc(sizeof(char*) * count);
	sub->command.details.sub.qoss = (int*) malloc(sizeof(int) * count);
	for (i = 0; i < count; ++i)
	{
...
int MQTTAsync_unsubscribeMany(MQTTAsync handle, int count, char* const* topic, MQTTAsync_responseOptions* response)
{
	...
	unsub = (MQTTAsync_queuedCommand*) malloc(sizeof(MQTTAsync_queuedCommand));
	...
	unsub->command.details.unsub.topics = (char**) malloc(sizeof(char*) * count);
	for (i = 0; i < count; ++i)
							 ...
							 int qos, int retained, MQTTAsync_responseOptions* response)
{
	...
	pub = (MQTTAsync_queuedCommand*) malloc(sizeof(MQTTAsync_queuedCommand));
...
int MQTTAsync_getPendingTokens(MQTTAsync handle, MQTTAsync_token **tokens)
{
	...
	*tokens = (MQTTAsync_token*) malloc(sizeof(MQTTAsync_token) * (count + 1));  /* add space for sentinel at end of list */
