...
int loadandcall(char* libname)
{
	...
	{	
		*(MQTTAsync_nameValue **) (&func_address) = (MQTTAsync_nameValue*) dlsym(APILibrary, "MQTTAsync_getVersionInfo");
		if (func_address == NULL)
		    *(MQTTAsync_nameValue **) (&func_address) = (MQTTAsync_nameValue*) dlsym(APILibrary, "MQTTClient_getVersionInfo");
		if (func_address)
			rc = printVersionInfo((*func_address)());
		dlclose(APILibrary);
	}
